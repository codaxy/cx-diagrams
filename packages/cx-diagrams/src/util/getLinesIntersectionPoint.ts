export interface IntersectionPoint {
    x: number;
    y: number;
}

export function getLinesIntersectionPoint(
    l1x1: number,
    l1y1: number,
    l1x2: number,
    l1y2: number,
    l2x1: number,
    l2y1: number,
    l2x2: number,
    l2y2: number
): IntersectionPoint | false {
    if (!isLineValid(l1x1, l1y1, l1x2, l1y2) || !isLineValid(l2x1, l2y1, l2x2, l2y2)) {
        return false;
    }

    const denominator = (l2y2 - l2y1) * (l1x2 - l1x1) - (l2x2 - l2x1) * (l1y2 - l1y1);

    if (isParallel(denominator)) {
        return false;
    }

    const ua = ((l2x2 - l2x1) * (l1y1 - l2y1) - (l2y2 - l2y1) * (l1x1 - l2x1)) / denominator;
    const ub = ((l1x2 - l1x1) * (l1y1 - l2y1) - (l1y2 - l1y1) * (l1x1 - l2x1)) / denominator;

    if (!isWithinSegment(ua, ub)) {
        return false;
    }

    const x = l1x1 + ua * (l1x2 - l1x1);
    const y = l1y1 + ua * (l1y2 - l1y1);

    return { x, y };
}

function isLineValid(x1: number, y1: number, x2: number, y2: number): boolean {
    return x1 !== x2 && y1 !== y2;
}

function isParallel(denominator: number): boolean {
    return denominator === 0;
}

function isWithinSegment(ua: number, ub: number): boolean {
    return ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1;
}
